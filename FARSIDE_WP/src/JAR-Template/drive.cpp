...
void Drive::control_reverse_arcade(bool engaged){
  double turnImportance = 0.5;
  double turnVal = controller1.Axis4.position();
  double forwardVal = controller1.Axis2.position();
  ...
  l_cata.spin(fwd, forwardVolts-turnVolts, volt);
  r_cata.spin(fwd, forwardVolts+turnVolts, volt);
  }
  else {
    DriveL.spin(fwd, forwardVolts-turnVolts, volt);
    DriveR.spin(fwd, forwardVolts+turnVolts, volt);
